...
export default function SlideViewPort({ children }: SlideViewPortProps) {
  ...
  const zoom = use$(editor$.visibleZoomLevel);
  const properties = use$(presentation$.properties);

  useObserve(() => {
    const dimension = dimension$.get();
    const properties = presentation$.properties.peek();
  ...
  return (
    <div className="bg-content2 p-20 grow flex items-center justify-center overflow-hidden">
      ...
      >
        <Stage width={properties.width} height={properties.height}>
